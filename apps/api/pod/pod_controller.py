...
class PodCreateController(PodBaseController):
    ...
    def before_handler(self, request, data, **kwargs):
        ...
        if deployment:
            labels.update({"deployment": deployment})
    ...
    def _format_env(self, envstring):
        ...
        try:
            ...
            raise exception_common.ValueValidateError("env不支持的解析格式")

    def _fetch_deployment(self, deployment):
        '''
        ...
        _info = {"id": deployment.get("id"),
                 "name": deployment["name"],
                 "kubernetes_url": deployment["kubernetes_url"],
                 "kubernetes_token": deployment["kubernetes_token"],
                 "kubernetes_ca": deployment["kubernetes_ca"],
                 "apiversion": deployment["apiversion"],
                 "selector": deployment["selector"],
                 "labels": deployment["labels"],
                 "namespace": deployment["namespace"],
                 "replicas": deployment["replicas"],
                 'docker_password': deployment["docker_password"],
                 'docker_username': deployment["docker_username"],
                 'docker_register_server': deployment["docker_register_server"],
                 'imagePullSecrets': deployment["imagePullSecrets"]
                 }

        build_info = {}
        build_info["image"] = deployment["image"]
        build_info["name"] = deployment["containername"]
        ...
        build_info["ports"] = [{"containerPort": deployment["containerports"]}]
        build_info["resources"] = {"requests": {"cpu": deployment.get("request_cpu", 0.01),
                                                "memory": str(deployment.get("request_memory", "256")) + "Mi"},
                                   "limits": {"cpu": deployment.get("limit_cpu", 1),
                                              "memory": str(deployment.get("limit_memory", "2048")) + "Mi"}
                                   }

        _info["containers"] = [build_info]
    ...
    def create(self, request, data, **kwargs):
        ...
        data = self.before_handler(request, data, **kwargs)
        create_data = self._fetch_deployment(data)
        ...
        if _res:
            time.sleep(0.1)
            pod = self.resource.detail(name=create_data["name"],
                                       kubernetes_url=create_data.get("kubernetes_url"),
                                       kubernetes_token=create_data.get("kubernetes_token"),
                                       kubernetes_ca=create_data.get("kubernetes_ca"),
                                       apiversion=create_data.get("apiversion"),
            ...
            raise exception_common.ResourceNotCompleteError(param="",
                                                            msg="pod %s 部署失败" % create_data["name"],
                                                            return_data=None)
